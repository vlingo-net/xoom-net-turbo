...
namespace Vlingo.Xoom.Turbo.Tests.Annotation.Codegen.Initializer
{
	public class XoomInitializerGenerationStepTest : IDisposable
	{
		...
		{
			var context = CodeGenerationContext.Using(_mockRepository.Create<FileStream>(It.IsAny<IntPtr>(), FileAccess.Read).Object, _mockRepository.Create<Type>().Object)
			...
			var xoomInitializer = context.FindContent(AnnotationBasedTemplateStandard.XoomInitializer, "XoomInitializer");
		...
		private void LoadContents(CodeGenerationContext context)
		{
			...
			context.AddContent(new TemplateStandard(TemplateStandardType.ProjectionDispatcherProvider),
				new OutputFile(PersistencePackagePath, "ProjectionDispatcherProvider.cs"), ProjectionDispatcherProviderContent);
		}

		private static readonly string HomeDirectory = Path.Combine(System.IO.Directory.GetCurrentDirectory(), "xoom-app");
