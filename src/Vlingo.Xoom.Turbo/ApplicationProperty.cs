...
namespace Vlingo.Xoom.Turbo
{
  public class ApplicationProperty
  {
    ...
    public static List<string> ReadMultipleValues(string key, string separator,
      IReadOnlyDictionary<string, string> properties)
    {
      var value = ReadValue(key, properties);
      return value == null ? new List<string>() : value.Split(new[] {separator}, StringSplitOptions.None).ToList();
    }
    ...
    private static string? RetrieveFromEnvironment(string key)
    {
      ...
      string value = EnvironmentVariables.Retrieve(envKey);
    ...
    private static string ResolveEnvironmentVariable(string key) => string.Format(CombinationPattern, XoomPrefix, key)
      .Replace(".", "_").ToUpper();
  }
}