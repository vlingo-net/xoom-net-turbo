...
namespace Vlingo.Xoom.Turbo.Scooter.Model
{
    public class Applied<S, C> where S : class where C : class
    {
        ...
        public Applied(int stateVersion, List<Source<C>> sources, Metadata metadata) : this(null, stateVersion, sources, metadata)
        {
        }
        ...
        public Applied(int stateVersion, List<Source<C>> sources) : this(null, stateVersion, sources, Metadata.NullMetadata())
        {
        }
        ...
        public Applied<S, C> AlongWith(int stateVersion, List<Source<C>> sources) => AlongWith(state, stateVersion, sources, metadata);

        public Applied<S, C> AlongWith(S state, int stateVersion, List<Source<C>> sources) => AlongWith(state, stateVersion, sources, metadata);
