...
namespace Vlingo.Xoom.Turbo.Exchange
{
  public class ExchangeSettings
  {
    ...
    public static ExchangeSettings Of(string exchangeName) =>
      _allExchangeParameters.FirstOrDefault(param => param.HasName(exchangeName)) ??
    ...
    {
      if (!_allExchangeParameters.Any())
      {
        Func<string, ExchangeSettings> mapper = exchangeNames => new ExchangeSettings(exchangeNames, properties);

        var exchangeParameters = ApplicationProperty
          .ReadMultipleValues(_exchangeNames, ";", properties).Select(mapper);
    ...
    private List<ExchangeSettingsItem> RetrieveParameters(IReadOnlyDictionary<string, string> properties) 
      => _keys
      .Select(key => new ExchangeSettingsItem(key, ApplicationProperty.ReadValue(key, properties) ?? string.Empty))
      .ToList();
